...
// Dev-only: if index.html changes, drop the template cache so nodemon-less
// template edits are picked up without a restart.
if (process.env.NODE_ENV !== 'production') {
  try {
    fsSync.watch(INDEX_PATH, { persistent: false }, () => {
      clearTemplateCache();
    });
  } catch (_) {
